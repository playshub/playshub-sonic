...
export interface JoinTelegramChatTaskDrawerProps {
  open: boolean;
  onClose: () => void;
  refetchProfile: () => void;
}

export default function JoinTelegramChatTaskDrawer({
  open,
  onClose,
  refetchProfile,
}: JoinTelegramChatTaskDrawerProps) {
  const { data, refetch } = useQuery({
    queryKey: ["get_quest_status"],
    queryFn: getQuestStatus,
  });
  const checkQuestMutation = useMutation({
    mutationFn: () => checkQuest("TASK", "JOIN_TELEGRAM_CHAT"),
  });
  const proceedQuestMutation = useMutation({
    mutationFn: () => proceedQuest("TASK", "JOIN_TELEGRAM_CHAT"),
  });

  const task = data?.find((item) => item.requestType === "JOIN_TELEGRAM_CHAT");
      ...
      <Flex vertical align="center" gap={15}>
        <Flex vertical align="center">
          <div style={{ padding: 10 }}>
            <Image src="/icons/earn/join-chat.png" preview={false} width={80} />
          </div>
