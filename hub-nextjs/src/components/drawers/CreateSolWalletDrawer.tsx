...
export interface CreateSolWalletDrawerProps {
  open: boolean;
  onClose: () => void;
  refetchProfile: () => void;
}

export default function CreateSolWalletDrawer({
  open,
  onClose,
  refetchProfile,
}: CreateSolWalletDrawerProps) {
  const { generateWallet } = useSolWallet();
  ...
  const checkQuestMutation = useMutation({
    mutationFn: () => checkQuest("TASK", "CREATE_SOL_WALLET"),
  });
  const proceedQuestMutation = useMutation({
    mutationFn: () => proceedQuest("TASK", "CREATE_SOL_WALLET"),
  });

  const task = data?.find((item) => item.requestType === "CREATE_SOL_WALLET");
      ...
      <Flex vertical align="center" gap={15}>
        <Flex vertical align="center">
          <div style={{ padding: 10 }}>
            <Image
              src="/icons/earn/create-wallet.png"
              preview={false}
              width={80}
            />
          </div>
